...
"""
    This is a PyTorch data augmentation library, which takes PyTorch Tensor as input,
...
def brightness_additive(tensor_img, std, mean=0, per_channel=False):
    ...
    if len(tensor_img.shape) == 5:
        ...
        raise ValueError('Invalid input tensor dimension, should be 5d for volume or 4d for 2d images')
...
"""
    1, C, D, H, W
"""
def gamma(tensor_img, gamma_range=(0.5, 2), per_channel=False, retain_state=False):
    ...
    minm, maxm = minm.unsqueeze(1), maxm.unsqueeze(1) # unsqueeze for brodcast machanism
...
def crop_2d(tensor_img, tensor_lab, crop_size, mode):
    assert mode in ['random', 'center'], "Invalid Mdoe, should be \'random\' or \'center\'"
